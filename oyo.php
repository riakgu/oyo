...
$page_api = file_get_contents('https://randomuser.me/api/?nat=us');

$domain = array ('@gmail.com','@yahoo.com','@hotmail.com','@yahoo.co.id');
        $random = rand(0,3);
		$first = fetch_value($page_api, '"first":"','"');
		$last = fetch_value($page_api, '"last":"','"');
		$email  = $first.'.'.$last.number(2).$domain[$random];
		$name = $first.' '.$last;
...
curl_setopt($ch, CURLOPT_URL, 'https://api.oyorooms.com/v2/users/generate_otp?phone='.$phone_number.'&nod=4&intent=login&sms_auto_retrieval=true&country_code=%2B'.$ct.'&version=20205&partner_app_version=20205&android_id='.generateRandomString(16).'&idfa=&sid=1551940465205');
...
if($json->success == 0){
	...
	echo " Nomer HP = $phone_number\r\n";
	echo " Refferal = $referral\r\n";
	echo " Email    = $email\r\n";
	echo " Nama     = $name\r\n";
	die();
} 
elseif($json->success == 1){
	echo $json->phone." GAGAL ANJING";
	die();
}